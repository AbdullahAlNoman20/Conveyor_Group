// FILE: src/pages/modules/manager/pages/ManagerPreBookings.jsx (FULL REWRITE — Accept/Reject, kitchen handoff, revenue estimate)
...
function priceForMealString(mealString, menu) {
  const names = String(mealString || "").split(",").map((s) => s.trim()).filter(Boolean);
...
export default function ManagerPreBookings() {
  ...
  // Manager uses to plan market/purchase quantities (SRS §10.3).
  const estimateSource = bookings.filter((b) => b.status === "confirmed" || b.status === "accepted");
  ...
  const totalEstimate = estimateSource.reduce((s, b) => s + priceForMealString(b.meal, menu), 0);
  ...
  async function accept(booking) {
    ...
    const order = {
      ...
      tableNumber: booking.collectionType === "dine_in" ? booking.tableNumber : null,
      orderType: booking.collectionType,
      priority: "normal",
      specialInstructions: `Pre-booked for ${booking.date}`,
      items: String(booking.meal || "").split(",").map((s) => s.trim()).filter(Boolean).map((name) => {
      ...
      tax: 0,
      amount,
      status: "pending", // goes straight into the Kitchen Queue
      preBookingId: booking.id,
      createdAt: new Date().toISOString(),
    };
    await dataStore.insert("orders", order);
    const nextBookings = await dataStore.update("preBookings", (b) => b.id === booking.id, { status: "accepted" });
    socket.emit(SOCKET_EVENTS.ORDER_SUBMITTED, {
      message: `Pre-booking for ${booking.date} accepted — sent to kitchen.`,
      recipientNames: [booking.clientName],
    });
    push(`Booking accepted — Tk ${amount} added to today's estimate, order sent to kitchen.`, "success");
    setBusyId(null);
  }

  async function reject(booking) {
    setBusyId(booking.id);
    await dataStore.update("preBookings", (b) => b.id === booking.id, { status: "rejected" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-400">
            Accept sends the order straight to the kitchen and counts it in the estimate below.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        <StatCard label="Total Pre-Booked Plates" value={totalPlates} Icon={CalendarClock} accent="brand" />
        <StatCard label="Estimated Revenue" value={`Tk ${totalEstimate.toLocaleString()}`} Icon={TrendingUp} accent="emerald" />
        <StatCard label="Dates Covered" value={Object.keys(byDate).length} accent="sky" />
      </div>

      {Object.keys(byDate).length > 0 && (
        <div className="rounded-xl border border-ink-100 bg-white p-5">
          <h2 className="mb-3 text-sm font-bold text-ink-700">Estimate by Date (for market/purchase planning)</h2>
          <div className="space-y-2">
            {Object.entries(byDate).sort(([a], [b]) => a.localeCompare(b)).map(([date, v]) => (
              <div key={date} className="flex items-center justify-between rounded-lg bg-ink-50 px-3 py-2 text-sm">
                <span className="font-medium text-ink-800">{date}</span>
                <span className="text-ink-500">{v.count} plate(s)</span>
                <span className="font-semibold text-brand-600">Tk {v.amount.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <tbody className="divide-y divide-ink-100">
            {pagedBookings.map((b) => (
              <tr key={b.id}>
                ...
                <td className="px-4 py-3 font-semibold text-ink-900">Tk {priceForMealString(b.meal, menu)}</td>
