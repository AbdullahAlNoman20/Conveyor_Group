  ...
  if (status === "cancelled" || status === "rejected") {
    return (
      <div className="flex items-center gap-2 rounded-lg bg-ink-100 px-4 py-3 text-sm text-ink-600">
        ...
        This order was{" "}
        {status === "rejected" ? "rejected by the Manager" : "cancelled"}.
      </div>
    );
  }
    ...
    <div>
      {status === "delayed" && (
        <div className="mb-3 flex items-center gap-2 rounded-lg bg-brand-50 px-3 py-2 text-xs font-medium text-brand-700">
          <AlertTriangle size={14} /> This order has been marked as delayed by
          the kitchen.
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              {i < ORDER_STEPS.length - 1 && (
                <div
                  className={`mx-1 h-0.5 flex-1 ${done ? "bg-emerald-500" : "bg-ink-100"}`}
                />
              )}
            </div>
          );
